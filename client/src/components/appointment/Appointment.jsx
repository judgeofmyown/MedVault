...
function Appointment() {
  ...
  const fetchHospitals = async () => {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        toast.warn("No hospitals found in this location.");
        setHospitals([]);
      }
    } catch (error) {
      console.error("Error fetching hospitals:", error);
      toast.error("Unable to fetch hospitals. Please try again.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Appointment booked successfully with ID: ${docRef.id}`);
  ...
  return (
    <div className="app">
      ...
      {showModal ? <Patient hospitalId={hospitalId} setShowModal={setShowModal} appointmentId={appointmentId} /> : null}
    </div>
  );
}
